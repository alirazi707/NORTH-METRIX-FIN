          ...
          {problems.map((problem) => (
            <StaggerItem key={problem.title}>
              <div className="group relative p-10 h-full rounded-[2rem] bg-white/5 border border-white/10 hover:border-white/30 hover:bg-white/10 hover:shadow-2xl hover:shadow-indigo-500/20 transition-all duration-500 hover:-translate-y-3">
                {/* Icon */}
                <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center mb-8 border border-white/10 group-hover:bg-white/20 transition-colors duration-500">
                  <problem.icon className="w-8 h-8 text-white group-hover:text-white transition-colors duration-500" />
                </div>

                {/* Content */}
                <h3 className="text-2xl font-medium text-white mb-4 group-hover:text-white/90 transition-colors">
                  {problem.title}
                </h3>
